...
func main() {
	/*
	people := PersonSlice{NewPerson("Yuchen", "Zhong"),
	   					NewPerson("Jackie","Ma"),
						NewPerson("Jianlin","Wang"),
						NewPerson("Jinping","Xi"),
						NewPerson("Huateng","Ma")}
	fmt.Println(people)
	sort.Sort(people)
	fmt.Println(people)*/

	res := Fold([]int{1,2,3,4}, 5, mul)
	fmt.Println(res)
}
...
func (p *Person) String() string{
	return fmt.Sprintf("(%v)%s %s",  p.ID, p.FirstName, p.LastName)
}
...
func (ps PersonSlice) Less(i, j int) bool{
	//升序
	if res := strings.Compare(ps[i].LastName, ps[j].LastName); res != 0 {
		return res < 0
	}
	if res := strings.Compare(ps[i].FirstName, ps[j].FirstName); res != 0 {
		return res < 0
	}
